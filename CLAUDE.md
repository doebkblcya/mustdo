...
> Todo Analyzer 代码库参考。详细产品文档见 `docs/PROJECT.md`。
...
Todo Analyzer — 轻量语音待办工具。语音只做"新增待办"，修改/删除/完成/置顶全部手动操作。核心原则：主流程短（按→说→松→入库）、语音不做危险操作、AI 结果不做确认弹窗。
...
│   ├── scripts/  (init_db.py, create_invite.py, list_invites.py, clear_invites.py, cleanup_overdue.py)
│   └── tests/
├── miniprogram/
...
### 语音/AI 数据流
小程序录音（16kHz/mono/PCM，上限 60s）→ `wx.uploadFile` → 后端 PCM→WAV→base64→火山引擎极速版 HTTP POST → transcript → DeepSeek JSON Output → 校验入库。失败不写数据库。

### 火山 ASR
端点 `POST .../api/v3/auc/bigmodel/recognize/flash`，资源 `volc.bigasr.auc_turbo`，同步接口。认证：新版 `X-Api-Key`。
...
- 测试：`PYTHONPATH=backend .venv/bin/python -m unittest discover -s backend/tests -v`。临时 SQLite + env patch
