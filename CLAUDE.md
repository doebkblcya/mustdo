...
### 处理面板（添加流程，processing-panel 组件）
- 语音/文字共用统一面板；**mode 是全局偏好**（设置页「自动添加/添加前确认」），进入面板时快照一次，语音与文字同值
- **四态共用同一收尾，无「已添加 N 项」成功卡**：保存成功即步骤条全绿（语音识别/文字输入 ✓ → AI 解析 ✓ → 保存待办 ✓）停留 1.3s 自动关闭
- **AI 解析步骤下逐条展开解析结果**（只读：内容 + 日期·时间），展示到面板关闭；自动模式解析出 0 条时该步骤下提示文案后自动关闭；保存待办步骤下不展示内容
- 确认模式在解析后停「确认待办」审核屏（可改原文/条目），确认保存后走同一全绿收尾
- 关闭入口（✕）只在「确认待办」与错误态；处理中/全绿停留期间点击面板不打断自动关闭
- 步骤条阶段（phase）：`uploading → transcribing → parsing → saving → saved(停留) / no_result(0条停留)`；`reviewing`（确认模式）、`error` 为分支态
...
### 卡片状态
- **done**：精确变灰（非整卡透明）—— 标题划线 `#b3b3b9`、meta `#c7c7cc`、右侧动作图标 `opacity:.35`，勾选圈实心黑；提醒按钮仅 pending 亮（done 无提醒入口）
...
- **内容**：只取今天「未完成」任务（不受 showCompleted 开关影响）；AI 视图内勾掉已完成后 380ms 移出视图
- **缓存**：`ORGANIZE_CACHE_KEY` 存 `{fingerprint, groups}`，任务集合未变时直接复用，不发请求
- **不自动重排**：视图内增删改只做就地重建（空组标题消失、新任务落「未分组」）；重新整理仅发生在退出重进（指纹变化触发）。手动「重新整理」入口已随旧切换条删除
...
### 底部弹层（编辑 / 提醒）
- 共享样式放在 `app.wxss` 的「Bottom sheet」分节（全局生效），`todos` / `trash` 两页不再各写一份；**不要**改成 `styles/*.wxss` + `@import`——新建目录后开发者工具可能不索引，会出现 `path ... not found from ...` 的 WXSS 编译错误
