...
8. 后端以事务写入 SQLite。
9. 前端刷新待办；处理面板步骤条全绿停留约 1.3s 后自动关闭回列表。
```

文字输入走同一链路：小程序键盘输入 → `POST /api/todos/parse`（`source=text`）→ `POST /api/todos/batch`。语音路径先经 ASR 得到 transcript，再进入同一 parse/batch 管道。

**处理面板收尾（语音/文字 × 自动/确认 四态共用）**：

- 全程以步骤条面板反馈：语音识别/文字输入（步骤下展示转写或输入内容）→ AI 解析（完成后步骤下逐条展开解析出的待办：内容 + 日期·时间，只读）→ 保存待办（步骤下无内容）。
- 自动模式：解析后直接入库；确认模式：解析后停在「确认待办」审核屏（改原文/逐条编辑后确认）。
- **两种模式保存后都不再弹「已添加 N 项」成功卡**：保存成功即三步全绿，停留约 1.3s 让用户看清条目后自动关闭回列表。
- 自动模式解析出 0 条：AI 解析步骤下展示提示文案（如「没有识别到需要新增的待办」），停留后自动关闭。
- 关闭入口（✕）只出现在「确认待办」和错误态；处理中与全绿停留期间点击面板不做任何事（不打断自动关闭）。
