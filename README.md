...
| 原则 | 说明 |
|------|------|
| **主流程要短** | 默认自动添加：按住说话 → 松手 → 自动转写、解析、入库 |
| **语音不做危险操作** | 不通过 AI 修改或删除已有事项，语音只负责新增 |
| **确认按需开启** | 可在设置中开启添加前确认，审核并编辑解析结果后再保存 |
...
| `POST` | `/api/todos/batch` | 批量新增最终待办条目 |
